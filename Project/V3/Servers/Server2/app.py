...
def setConnection():
    try:
        engine = sqlalchemy.create_engine('mysql://root:my-secret-pw@172.18.0.3')  # connect to server
...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:my-secret-pw@172.18.0.3/expensesdb'
...
class expenses(db.Model):
    ...
    def __init__(self, _id, name, email, category, description, link, estimated_costs, submit_date, status,
                 decision_date):
        self._id = _id
        self.name = name
        self.email = email
...
def expenses_post():
    ...
    expense = expenses(content['id'], content['name'], content['email'], content['category'], content['description'],
                       content['link'],
                       content['estimated_costs'], submit_date.strftime('%Y-%m-%d'), 'pending', datetime.date.today())
    db.session.add(expense)
    db.session.commit()
    content['id'] = expense._id
...
def expenses_get(_id):
    expense = expenses.query.filter_by(_id=_id).first()
    if expense:
        content = {}
        content['id'] = str(expense._id)
...
@app.route('/v1/expenses/<int:_id>', methods=['PUT'])
def expenses_put(_id):
    expense = expenses.query.filter_by(_id=_id).first()
    if expense:
        content = request.get_json(force=True)
        for key in content:
            db.engine.execute(
                "UPDATE expenses SET " + str(key) + " = " + str(content[key]) + " WHERE _id = " + str(
                    _id))
        return '', 202
    else:
        return '', 404
...
@app.route('/v1/expenses/<int:_id>', methods=['DELETE'])
def expenses_del(_id):
    expense = expenses.query.filter_by(_id=_id).first()
    if expense:
