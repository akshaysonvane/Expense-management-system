...
def expenses_post():
    ...
    expense = expenses(content['name'], content['email'], content['category'], content['description'], content['link'],
...
def expenses_put(expense_id):
    ...
    if expense:
        ...
        for key in content:
            db.engine.execute(
                "UPDATE expenses SET " + str(key) + " = " + str(content[key]) + " WHERE expense_id = " + str(
                    expense_id))
        return '', 202
    else:
        return '', 404
